...
as the curl of a gradient is always zero @eq-curl-grad-zero. Note that this is the second of the Maxwell's equations (@eq-Maxwell-2) for the steady-state case (i.e. $\frac{\partial \bfB}{\partial t} = 0$).
...
$$ W = - \int_C \bfF \cdot \dd \bfs = - q \int_C \bfE \cdot \dd \bfs = q \int_C \nabla \phi \cdot \dd \bfs = q \left[ \phi (\bfr_\mathrm{f}) - \phi (\bfr_\mathrm{i})\right].$$ {#eq-work-electrostatic}
