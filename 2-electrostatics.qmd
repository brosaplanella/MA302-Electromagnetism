...
* Understand Coulomb's law and apply it to calculate forces between point charges.

* Explain the concepts of electric field, electrostatic potential and electrostatic energy, both for point charges and charge distributions.

* Understand Gauss' law both for point charges and charge distributions.

:::

We start our journey in the field of electromagnetism with electrostatics, which is the branch of the subject concerned with stationary electric charges. This is the simplest case, but it is present in our day-to-day lives in many ways: from laser printing to explaining static cling (see @fig-examples-electrostatics).
...
As you may already know, subatomic particles like the proton and electrons, have a physical property called _electric charge_. Electric charge can be positive or negative and it is _quantised_, that means that it comes in integer multiples of the elementary charge $e$ which, in SI units, is defined as $e \approx 1.602 \times 10^{-19} \; \mathrm{C}$ (this unit is called Coulomb^[The unit is named after the french physicist Charles-Augustin de Coulomb, who formulated what today is known as Coulomb's law (coming next).]). Protons have a charge of $+e$ while electrons have a charge of $-e$, and all charges in matter arise from them. Charges^[As a shorthand, we will refer to particles with an electrical charge as "charges".] with the same sign attract each other, while charges of opposite sign repel each other. 

We want to model the force between point charges. By point charges we mean that we will represent these electrically charges as points in $\mathbb{R}^3$, with their position in space denoted by the vector $\bfr \in \mathbb{R}^3$ and (the magnitude and sign of) their charge denoted by $q_i$. 
...
Let's now introduce the concept of _electric field_. Even though it looks like a simple rewriting of the equation, it provides a very valuable new point of view. As you may recall, the electric field was one of the variables appearing in the Maxwell's equations.
...
::: {#rem-electric-field}
Note that $\bfF = q \bfE$ yields @eq-superposition. This expression also allows us to deduce that the the electric field is measured (in SI units) in $\mathrm{N} \; \mathrm{C}^{-1}$ (or, equivalently $\mathrm{V} \; \mathrm{m}^{-1}$).
:::
...
For any closed surface $\Sigma = \partial \Omega$ (that is the surface bounding a region $\Omega \subset \mathbb{R}^3$), we have

$$\int_\Sigma \bfE \cdot \bfn \; \dd A = \frac{1}{\epsilon_0} \sum_{i=1}^N q_i,$$ {#eq-Gauss-law}

where $q_i$ is the (finite^[We will consider what happens with infinite amounts of charge in the next section.]) set of charges contained in $\Omega$. For convenience, we define the total charge in $\Omega$ as $Q = \sum_{i=1}^N q_i$.
...
$$ \int_{\partial B_R} \bfE \cdot \frac{\bfr}{|\bfr|} \; \dd A = \frac{q}{4 \pi \epsilon_0} \int_{\partial B_R} \frac{1}{|\bfr|^2} \dd A = \frac{q}{4 \pi R^2 \epsilon_0} \int_{\partial B_R} \dd A = \frac{q}{\epsilon_0}.$${#eq-gauss-law-proof-integral-BR}
...
Now consider a distribution of $N$ charges $q_i$ located at $\bfr_i$, respectively. By the principle of superposition @eq-electric-field we can write the electric field as the sum of the contributions for each charge. Then, similar to @eq-gauss-law-proof-div-E, we can show that the divergence of the electric field will be zero in $\mathbb{R}^3 \setminus \bigcup_i \bfr_i$. Defining as $B_i$ as the ball of arbitrary radius $R$ centred at $\bfr_i$ (i.e. around charge $q_i$), we can write 
...
So far we have worked with a set of point charges. However, this is quite often impractical. For example, a macroscopic object has an enormous number of electrons and protons so it is not reasonable to consider them one by one. A more subtle point is that, from a quantum mechanics point of view, the position of electron cannot be determined and instead they should be treated as a "blur" of charge. In either case, the concept of _charge density_ comes handy.

::: {#def-charge-density}

## Charge density

We define the _charge density_ as a function $\rho(\bfr): \Omega \subset \mathbb{R}^3 \to \mathbb{R}$ that gives the charge per unit volume at a certain point in space.
...
Given a charge density $\rho : \Omega \subset \mathbb{R}^3 \to \mathbb{R}$, we define the _electric field_ at a given point $\bfr \in \mathbb{R}^3$ as
...
where $\dd V'$ represents the infinitesimal volume element associated to the coordinate $\bfr'$.
...
It's worth pointing out that, as $\rho$ has compact support, $\phi \sim |\bfr|^{-1}$ as $|\bfr| \to \infty$ and thus it vanishes.

:::

The proof also follows directly from @def-electric-field-charge-density. Note that, when taking the gradient of $\phi$, it acts on $\bfr$ only ($\bfr'$ is the integration variable and thus it's "invisible" from outside the integral).
...
We do not provide the detailed proof, but it consists on taking the continuum limit (i.e. $N \to \infty$) of @eq-Gauss-law. 
...
Using the divergence theorem (@thm-divergence) on @eq-Gauss-law-charge-density and rearranging we can write
...
::: {#exm-cylindrical-charge-density}

## Cylindrical charge density

::: {#fig-cylinder-charge}

![](./figs/cylinder_charge.svg)

Diagram of the cylindrical charge density and of the surface $\Sigma$ used to apply Gauss' law.
:::

Consider an infinitely long cylinder along the $z$ axis and with radius $r_0$. The cylinder has uniform charge $\rho$ and there is no charge outside of it.We want to compute the electric field generated by the cylinder using Gauss' law. 

First let's deduce the form of the electric field. The problem is invariant under rotations around and translations along the $z$ axis. Therefore, in cylindrical coordinates, we must have $\phi(r, \theta, z) = \phi(r, \theta + \theta_0, z + z_0)$ for all $\theta_0$ and $z_0$. From this we deduce that the potential can only depend on the radial component, i.e. $\phi = \phi(r)$. As $\bfE = - \nabla \phi$ we deduce that

$$\bfE = E(r) \bfe_r,$$

i.e. the electric field will point in the radial direction and depend only on $r$. 

Now consider a surface $\Sigma = \partial \Omega$ where $\Omega$ is a cylinder of arbitrary radius $R$ along the $z$ axis with height $H$ (as the problem is invariant under translations along $z$, it does not matter where we place the cylinder). The flux through the surface is

$$\int_\Sigma \bfE \cdot \bfn \; \dd A = \int_\Sigma E(r) \bfe_r \cdot \bfe_r \; \dd A = E(R) 2 \pi R H.$$

As the electric field is parallel to the top and bottom lids, it does not contribute to the flux in those surfaces. 

Now consider the total charge enclosed by $\Sigma$. If $R < r_0$, the total charge is

$$\int_\Omega \rho \; \dd V = \rho \pi R^2 H,$$

while if $R \geq r_0$ the total charge is

$$\int_\Omega \rho \; \dd V = \rho \pi r_0^2 H.$$


Using Gauss' law, for $R < r_0$ we find

$$ E(R) 2 \pi R H = \frac{1}{\epsilon_0} \rho \pi R^2 H,$$

therefore the electric field is $E(R) = \frac{\rho R}{2 \epsilon_0}$,

while for $R \geq r_0$ we find

$$ E(R) 2 \pi R H = \frac{1}{\epsilon_0} \rho \pi r_0^2 H,$$

therefore the electric field is $E(R) = \frac{\rho r_0^2}{2 \epsilon_0 R}$.

Combining the results (and relabelling $R$ into $r$ for convenience) we find that the electric field is

$$ \bfE = \begin{cases}
\frac{\rho r}{2 \epsilon_0}\bfe_r, & \text{if} \quad r < r_0,\\
\frac{\rho r_0^2}{2 \epsilon_0 r}\bfe_r, & \text{if} \quad r \geq r_0.
\end{cases}$$
...
In words, it is a function that is equal to zero everywhere except at the origin (where it is infinitely large), and its integral over any interval $I$ containing the origin^[This includes $\mathbb{R}$.] is equal to one.
...
For Gauss' law, we can substitute @eq-point-charge-to-density into @eq-Gauss-law-charge-density and obtain
...
as the curl of a gradient is always zero (@eq-curl-grad-zero). Note that this is the third of the Maxwell's equations (@eq-Maxwell-2) for the steady-state case (i.e. $\frac{\partial \bfB}{\partial t} = 0$).
...
$$ W = - \int_C \bfF \cdot \bft \; \dd s = - q \int_C \bfE \cdot \bft \; \dd s = q \int_C \nabla \phi \cdot \bft \; \dd s = q \left[ \phi (\bfr_\mathrm{f}) - \phi (\bfr_\mathrm{i})\right].$$ {#eq-work-electrostatic}
...
which we deduce from the principle of superposition ($\phi_2$ is the potential of the charge $q_2$). The total work done so far is $W = W_2 + W_3$. By induction, we can infer that for $N$ charges $q_1, \dots, q_N$ at $\bfr_1,\dots,\bfr_N$, respectively, the total work to assemble them is
...
$$W = \frac{1}{2} \int_\Omega \rho \phi \; \dd V.$$ {#eq-work-charge-density-1}

Using Gauss' law (@eq-Gauss-law-differential) and @eq-div-scalar-times-vector, we obtain
