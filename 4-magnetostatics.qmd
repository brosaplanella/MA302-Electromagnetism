...
* Explain the concepts of electric current and current density.

* Calculate the Lorentz force on a moving charge.

* Apply the Biot-Savart law and Ampère's law to calculate magnetic fields.

* Work with the vector potential and its gauge transformations.
...
At this point it is worth making a clarification. Very conveniently, the word \emph{flux} has two definitions. From a transport phenomena point of view, a flux is the rate of flow per unit area of a certain quantity, which is the case of the current density. Mathematically, a flux is the surface integral of a vector field (@def-flux), which is the case of the current. Normally it will be apparent from the context which definition is being used (otherwise it will be explicitly mentioned).
...
A very important property of electric charge is that it is conserved. Like with mass conservation, this means that charge cannot be created nor destroyed. The idea of conservation of _stuff_ underpins many areas of physics^[You will encounter it in quite a few modules, e.g. MA3D1.] and, luckily for us, we have a very convenient way of representing it mathematically.

The equation that encapsulates conservation of _stuff_ is called the continuity equation. Even though it is a very general equation, here we will state it and derive it in the context of electric fields.^[The generic form is derived in MA3D1.]
...
Consider a region $\Omega \in \bbR^3$, bounded by the surface $\partial \Omega$. The total charge in $\Omega$ is given by
...
$$\bfB(\bfr) = \frac{\mu_0 I}{4 \pi} \int_C \frac{\dd \bfr' \times (\bfr - \bfr')}{|\bfr - \bfr'|^3},$$ {#eq-Biot-Savart-wire}
...
A current $I$ through an infinitely long straight wire.
:::

Let's consider a steady current $I$ through an infinitely long straight wire. Wlog we will assume the wire is along the $z$ axis (i.e. $x=y=0$), as shown in @fig-wire-1. Then, we can work in cylindrical coordinates $(r, \theta, z)$. In that case, the differential of length in the integral can be written as $\dd \bfr' = \bfe_z \dd z'$. Then, we have

$$ \dd \bfr' \times (\bfr - \bfr') = r \; \dd z' \; \bfe_\theta,$$

i.e. the magnetic field will point in the angular direction. Then, from @eq-Biot-Savart-wire we have 

$$ \bfB = \frac{\mu_0 I}{4 \pi} \bfe_\theta \int_{-\infty}^{\infty} \frac{r}{(r^2 + (z-z')^2)^\frac{3}{2}} \; \dd z' = \frac{\mu_0 I}{2 \pi r} \bfe_\theta.$$

:::

## Gauss' law for magnetism

Let's now derive Gauss' law for magnetism which, as the name suggests, it is very similar to Gauss' law for electric fields (@thm-Gauss-law-diff).
...
First note that, using @eq-div-cross-product, we get
...
As $\bfv$ does not depend on $\bfr$ the first term vanishes. Now recall from @eq-grad-1-r that

$$\frac{\bfr}{|\bfr|^3} = - \nabla \frac{1}{|\bfr|},$$

so the second term can be written as the curl of a gradient. Therefore, from @eq-curl-grad-zero, we conclude
...
Historically, this was derived by Ampère (and others) using experimental and theoretical tools. However, we can derive it from the Biot-Savart law (@eq-Biot-Savart-J). We need to introduce a couple of lemmas first.
...
&= -\frac{\mu_0}{4 \pi} \int_\Omega \nabla' \cdot \left( \frac{\bfJ(\bfr')}{|\bfr - \bfr'|} \right) \dd V' + \frac{\mu_0}{4 \pi} \int_\Omega \frac{1}{|\bfr - \bfr'|} \left(  \nabla' \cdot \bfJ(\bfr') \right) \dd V'.
\end{aligned}
$$
...
$$ -\frac{\mu_0}{4 \pi} \int_\Omega \nabla' \cdot \left( \frac{\bfJ(\bfr')}{|\bfr - \bfr'|} \right) \dd V' = -\frac{\mu_0}{4 \pi} \int_{\partial \Omega} \frac{\bfJ(\bfr')}{|\bfr - \bfr'|} \cdot \bfn \; \dd A'.$$
...
$$ \int_C \bfB \cdot \bft \; \dd s = \mu_0 \int_\Sigma \bfJ \cdot \bfn \; \dd A = \mu_0 I,$$
...
A current $I$ through an infinitely long straight wire, but now we approach it by computing the integral along the curve $C$.
:::
...
For our curve $C$ we choose a circle of arbitrary radius $r$ in the $r-\theta$ plane with the centre on the $z$ axis. @cor-Ampere-law-alternative gives

$$ \int_C \bfB \cdot \bft \; \dd s =  \int_0^{2\pi} B(r) r \; \dd \theta = 2 \pi r B(r) = \mu_0 I,$$
