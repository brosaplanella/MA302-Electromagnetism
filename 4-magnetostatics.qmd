...
using `CROSSREF`, but as $\bfv$ does not depend on $\bfr$ the first term vanished. Recall from `CROSSREF` that
...
Using the identity `CROSSREF`, which applies to any vector field $\bfA$, we obtain
...
We have used `CROSSREF` in the second step.
...
Note as well that by defining the magnetic field from the vector potential it automatically satisfies $\nabla \cdot \bfB = 0$, as the divergence of a curl is always zero. The implication works both ways and any field $\bfB$ that can be written as the curl of a potential satisfies $\nabla \cdot \bfB = 0$. `CROSSREF`
