...
\begin{axis}[
    ...
    \addplot3[black, thick, dashed, variable=\t, domain=0:-3, samples y=1] ( {-cos(pi*\vartangent)}, {-sin(pi*\vartangent)}, {cos(-cos(pi*\vartangent)) + cos(-sin(pi*\vartangent)) + t});
    \draw[-Stealth, thick, black] (axis cs:0,0,2) -- (axis cs: 0,0,4.5) node[left] {$\boldsymbol{n}^+$}; 
    \draw[-Stealth, thick, black] (axis cs:0,0,2) -- (axis cs: 0,0,-1) node[right] {$\boldsymbol{n}^-$};
